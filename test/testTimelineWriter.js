...
describe('TimelineWriter', function() {
	...
	describe('writeSVG()', function() {
		for (const test of tests) {
			...
			const expectedPath = path.join(test.buildDir, `${test.procedure.filename}.summary.timeline.svg`);
			const testPath = path.join(test.buildDir, `test${test.procedure.filename}.summary.timeline.svg`);
	...
	describe('writePNG()', function() {
		for (const test of tests) {
			...
			const expectedPath = path.join(test.buildDir, `${test.procedure.filename}.summary.timeline.png`);
			const testPath = path.join(test.buildDir, `test${test.procedure.filename}.summary.timeline.png`);
			...
			it(`should create expected PNG for ${test.file}`, function(done) {
				test.timeline.writePNG(testPath, function() {
					...
					const msg = (expectedPNG === testPNG) ?
						goodMsg :
						'PNG generated by tests DOES NOT match expected';
